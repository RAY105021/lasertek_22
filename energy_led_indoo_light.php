<!doctype html>
<html lang="en" class="no-js">

<head>
    <title>能源事業 LED燈具系列</title>
    <meta charset="UTF-8">
    <meta name="keywords" content="LASER TEK, lasertek, 雷科股份有限公司, 能源事業,  LED室內燈, LED室外燈,  LED 特殊燈具, RAYCO8500高效空氣清淨機, 空氣清淨機濾網, 線上購物" />
    <meta name="description" content="能源事業 LED室內燈系列" />
    <meta name="viewport" content="width=device-width, user-scalable=no, initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
    <meta http-equiv="content-language" content="zh-tw">
    <link rel="Shortcut Icon" type="image/x-icon" href="favicon.ico" />
    <link rel="stylesheet" href="css/reset.css"> <!-- CSS reset -->
    <link rel="stylesheet" href="css/style.css"> <!-- Resource style -->
    <link rel="stylesheet" href="css/animations.css"> <!-- Animation style -->
    <link rel="stylesheet" href="css/section_index.css"> <!-- 首頁內容 style -->
    <link rel="stylesheet" href="css/balloon.css"> <!-- 提示框 style -->
    <script src="js/modernizr.js"></script> <!-- Modernizr -->
    <!--動畫使用-->
    <link rel="stylesheet" href="css/animate.css">
    <script src="js/wow.min.js"></script>
    <script>
        if (!(/msie [6|7|8|9]/i.test(navigator.userAgent))) {
            new WOW().init();
        };
    </script>
    <!--內頁專用-->
    <link rel="stylesheet" href="css/section_page.css"> <!-- 首頁內容 style -->


    <!--客服浮動+回上一頁-->
    <script type="text/javascript" src="js/service.js"></script>
    <!-- Global site tag (gtag.js) - Google Analytics -->
    <script async src="https://www.googletagmanager.com/gtag/js?id=UA-82641040-3"></script>
    <script>
        window.dataLayer = window.dataLayer || [];

        function gtag() {
            dataLayer.push(arguments);
        }
        gtag('js', new Date());

        gtag('config', 'UA-82641040-3');
    </script>

    <!-- 隱私權政策 -->
    <script id="cookieWarn" src="js/cookie-warn.js" data-debug="true"></script>


</head>

<body class="nav-is-fixed">

    <h1>能源事業 LED室內燈系列</h1>

    <!-- header  -->
    <?php include 'header.php'; ?>

    <main class="cd-main-content">

        <!-- 主要內容表列 -->

        <!-- 首頁專用橫幅 -->

        <div class="paralasic_energy" data-paralasic="0.4">
            <h2 class="wow fadeInDown">持續創新改善 全員追求卓越</h2>
            <h5 class="wow fadeInDown">Continuous innovation and improvement <br>All employees pursue excellence</h5>
        </div>


        <!-- 各單位專用橫幅 -->
        <!--
        <div id="paralasic02">
            <video class="masthead-video" autoplay loop muted poster="img/index_laser.jpg">
                <source src="mov/laser.mp4" type="video/mp4" width="auto">
                <source src="mov/laser.webm" type="video/webm" width="auto">
            </video>
            <h2 class="wow fadeInDown">持續創新改善 全員追求卓越</h2>
        </div>
         -->
        <!-- 
             配合以上橫幅
             影片使用 class="ltt_container"
             圖片使用 class="ltt_container_img"
         -->

        <div class="ltt_container_img">



            <!--產品表列-->
            <section class="ltt_news_all02">

                <!--麵包屑-->
                <div id="breadcrumbs">
                    <ul>
                        <li><a href="index.php">首頁</a> > </li>
                        <li><a href="#">部門資訊</a> > </li>
                        <li><a href="energy.php">能源事業</a> > </li>
                        <li><a href="energy_led_indoo_light.php">LED燈具系列</a> </li>
                        <li></li>
                    </ul>
                </div>


                <!--產品選單-->
                <div class="product_menu">
                    <ul class="product_menu_ul">

                        <li class="product_select33 product_menu_light_energy product_menu_width">
                            <div class="product_menu_table">
                                <div class="product_menu_table_img"><img src="img/energy/product_select_01.png" alt="LED燈具系列"></div>
                                <div class="product_menu_table_select">
                                    <div class="product_select100 typeface_bold_white">LED燈具系列</div>
                                    <div class="product_select100">
                                        <ul class="product_menu_select_200">
                                            <li>
                                                <select class="select-css" onChange="location = this.options[this.selectedIndex].value;">
                                                    <option value="#">選擇產品</option>
                                                    <option value="energy_led_indoo_light.php" class="product_menu_select_class" selected="true"><samp>LED燈具系列 ALL</samp></option>
                                                    <option value="energy_led_clean_room.php"> • 半導體無塵室專用LED黃光燈管</option>
                                                    <option value="energy_led_t8_lamp.php"> • LED高效T8燈管</option>
                                                    <option value="energy_led_t5_electronic.php"> • LED電子式T5燈管</option>
                                                    <option value="energy_led_t5_laminate.php"> • LED T5層板串接燈管</option>
                                                    <option value="energy_led_panel_light.php"> • LED超薄高效平板燈</option>
                                                    <option value="energy_led_grille_lam.php"> • LED格柵燈</option>
                                                    <option value="energy_led_patio_lights.php"> • LED天井燈</option>
                                                    <option value="energy_led_projection_light.php"> • LED泛光型</option>
                                                    <option value="energy_led_fish_light.php"> • LED COB投射燈</option>
                                                    <option value="energy_led_big_bulb_light.php"> • LED各式球泡燈</option>
                                                    <option value="energy_led_recessed_light.php"> • LED崁燈</option>
                                                    <option value="energy_led_chandelier_high_power.php"> • LED高功率崁燈</option>
                                                    <option value="energy_led_ar111.php"> • LED AR111 </option>
                                                </select>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </li>

                        <li class="product_select33 product_menu_deep_energy product_menu_width">
                            <div class="product_menu_table">
                                <div class="product_menu_table_img"><img src="img/energy/product_select_05.png" alt="濾網"></div>
                                <div class="product_menu_table_select">
                                    <div class="product_select100 typeface_bold_white">空氣清淨機 & 濾網</div>
                                    <div class="product_select100">
                                        <ul class="product_menu_select_200">
                                            <li>
                                                <select class="select-css" onChange="location = this.options[this.selectedIndex].value;">
                                                    <option value="#" selected="true">選擇產品</option>
                                                    <option value="energy_air_purifier_all.php" class="product_menu_select_class"><samp>空氣清淨機&濾網 ALL</samp></option>
                                                    <option value="energy_air_purifier_rayco8500.php"> • RAYCO8500高效空氣清淨機</option>
                                                    <option value="energy_aione_dust_filter.php"> • 抗敏性預過濾網 </option>
                                                    <option value="energy_epidemic_90_days.php"> • 90天長效抗菌防護濾膜</option>
                                                    <option value="energy_aione_hepa_filter.php"> • HEPA高效濾網</option>
                                                    <option value="energy_aione_active_charcoal_filter.php"> • 活性碳濾網</option>
                                                    <option value="energy_aione_photocatalyst_filter.php"> • 光觸媒濾網</option>
                                                    <option value="energy_aione_uv.php"> • 紫外線UV燈管</option>
                                                </select>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </li>

                        <li class="product_select33 product_menu_light_energy product_menu_width">
                            <div class="product_menu_table">
                                <div class="product_menu_table_img"><img src="img/energy/product_select_06.png" alt="GERMAGIC抗菌防疫"></div>
                                <div class="product_menu_table_select">
                                    <div class="product_select100 typeface_bold_white">GERMAGIC抗菌防疫</div>
                                    <div class="product_select100">
                                        <ul class="product_menu_select_200">
                                            <li>
                                                <select class="select-css" onChange="location = this.options[this.selectedIndex].value;">
                                                    <option value="#" selected="true">選擇產品</option>
                                                    <option value="energy_epidemic_products_all.php" class="product_menu_select_class"><samp>防疫產品 ALL</samp></option>
                                                    <option value="energy_epidemic_90_days.php"> • 90天長效抗菌防護濾膜</option>
                                                    <option value="energy_epidemic_72_hours_50ml.php"> • 72小時長效抗菌防護噴劑 50ML</option>
                                                    <option value="energy_epidemic_72_hours_200ml.php"> • 72小時長效抗菌防護噴劑 200ML</option>
                                                    <option value="energy_epidemic_72_hours_1000ml.php"> • 72小時長效抗菌防護噴劑 1000L</option>
                                                    <option value="energy_epidemic_90_days_return_air.php"> • 90天輕鋼架迴風口濾網組</option>

                                                </select>
                                            </li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
                        </li>

                    </ul>
                </div>


                <h3 av-animation="fadeInDown">LED燈具系列</h3>
                <h4 av-animation="fadeInUp">現場免費評估、規劃節能方案及客製化產品服務</h4>

                <div class="ltt_box_content_all">

                    <div class="wow slideInRight ltt_box_product_33 ltt_box_product_right">
                        <div class="ltt_box_laser_05 ltt_box_product_img02">
                            <a href="energy_led_clean_room.php"><img class="ltt_box_product_img" src="img/energy/product_thick_film01.png" alt="LED半導體無塵室黃光燈管"></a>
                            <p><a href="energy_led_clean_room.php">LED半導體無塵室黃光燈管</a></p>
                        </div>
                        <p class="ltt_box_product_description">
                            半導體無塵室專用
                        </p>
                    </div>

                    <div class="wow slideInLeft ltt_box_product_33 ltt_box_product_right">
                        <div class="ltt_box_laser_05 ltt_box_product_img02">
                            <a href="energy_led_t8_lamp.php"><img class="ltt_box_product_img" src="img/energy/product_thick_film04.png" alt="LED高效T8燈管"></a>
                            <p><a href="energy_led_t8_lamp.php">LED高效T8燈管</a></p>
                        </div>
                        <p class="ltt_box_product_description">
                            取代傳統日光燈管
                        </p>
                    </div>

                    <div class="wow slideInRight ltt_box_product_33 ltt_box_product_bottom">
                        <div class="ltt_box_laser_05 ltt_box_product_img02">
                            <a href="energy_led_t5_electronic.php"><img class="ltt_box_product_img" src="img/energy/product_thick_film02.png" alt="LED 電子式T5燈管"></a>
                            <p><a href="energy_led_t5_electronic.php">LED 電子式T5燈管</a></p>
                        </div>
                        <p class="ltt_box_product_description">
                            可直接安裝於傳統電子式T5燈座上
                        </p>
                    </div>

                    <div class="wow slideInLeft ltt_box_product_33 ltt_box_product_right">
                        <div class="ltt_box_laser_05 ltt_box_product_img02">
                            <a href="energy_led_t5_laminate.php"><img class="ltt_box_product_img" src="img/energy/product_thick_film03.png" alt="LED T5層板串接燈管"></a>
                            <p><a href="energy_led_t5_laminate.php">LED T5層板串接燈管</a></p>
                        </div>
                        <p class="ltt_box_product_description">
                            塑膠+鋁合金 防火等級V2
                        </p>
                    </div>

                    <div class="wow slideInRight ltt_box_product_33 ltt_box_product_right">
                        <div class="ltt_box_laser_05 ltt_box_product_img02">
                            <a href="energy_led_panel_light.php"><img class="ltt_box_product_img" src="img/energy/product_thick_film05.png" alt="LED 超薄高效平板燈"></a>
                            <p><a href="energy_led_panel_light.php">LED 超薄高效平板燈</a></p>
                        </div>
                        <p class="ltt_box_product_description">
                            超薄高亮度光源全面性
                        </p>
                    </div>


                    <div class="wow slideInleft ltt_box_product_33 ltt_box_product_bottom">
                        <div class="ltt_box_laser_05 ltt_box_product_img02">
                            <a href="energy_led_grille_lam.php"><img class="ltt_box_product_img" src="img/energy/product_thick_film06.png" alt="LED 格柵燈"></a>
                            <p><a href="energy_led_grille_lam.php">LED 格柵燈</a></p>
                        </div>
                        <p class="ltt_box_product_description">
                            可取代整組傳統螢光燈管格柵燈
                        </p>
                    </div>


                    <div class="wow slideInright ltt_box_product_33 ltt_box_product_right">
                        <div class="ltt_box_laser_05 ltt_box_product_img02">
                            <a href="energy_led_patio_lights.php"><img class="ltt_box_product_img" src="img/energy/product_thick_film13.png" alt="LED 天井燈"></a>
                            <p><a href="energy_led_patio_lights.php">LED 天井燈</a></p>
                        </div>
                        <p class="ltt_box_product_description">
                            矽橡膠圈雙重防水式設計
                        </p>
                    </div>


                    <div class="wow slideInLeft ltt_box_product_33 ltt_box_product_right">
                        <div class="ltt_box_laser_05 ltt_box_product_img02">
                            <a href="energy_led_projection_light.php"><img class="ltt_box_product_img" src="img/energy/product_thick_film14.png" alt="LED 投射燈"></a>
                            <p><a href="energy_led_projection_light.php">LED 泛光型</a></p>
                        </div>
                        <p class="ltt_box_product_description">
                            耐用性高的鋁合金外殼
                        </p>
                    </div>


                    <div class="wow slideInRight ltt_box_product_33 ltt_box_product_bottom">
                        <div class="ltt_box_laser_05 ltt_box_product_img02">
                            <a href="energy_led_fish_light.php"><img class="ltt_box_product_img" src="img/energy/product_thick_film15.png" alt="LED 投射燈(無紫外線)"></a>
                            <p><a href="energy_led_fish_light.php">LED COB投射燈</a></p>
                        </div>
                        <p class="ltt_box_product_description">
                            無紫外光及其他有害光線
                        </p>
                    </div>


                    <div class="wow slideInLeft ltt_box_product_33 ltt_box_product_right">
                        <div class="ltt_box_laser_05 ltt_box_product_img02">
                            <a href="energy_led_big_bulb_light.php"><img class="ltt_box_product_img" src="img/energy/product_thick_film07.png" alt="LED 大球泡燈"></a>
                            <p><a href="energy_led_big_bulb_light.php">LED 各式球泡燈</a></p>
                        </div>
                        <p class="ltt_box_product_description">
                            5700K / 3000K
                        </p>
                    </div>

                    <div class="wow slideInRight ltt_box_product_33 ltt_box_product_right">
                        <div class="ltt_box_laser_05 ltt_box_product_img02">
                            <a href="energy_led_recessed_light.php"><img class="ltt_box_product_img" src="img/energy/product_thick_film08.png" alt="LED 崁燈"></a>
                            <p><a href="energy_led_recessed_light.php">LED 崁燈</a></p>
                        </div>
                        <p class="ltt_box_product_description">
                            一體化被動式散熱，無傳導熱牆效應
                        </p>
                    </div>


                    <div class="wow slideInLeft ltt_box_product_33 ltt_box_product_bottom">
                        <div class="ltt_box_laser_05 ltt_box_product_img02">
                            <a href="energy_led_chandelier_high_power.php"><img class="ltt_box_product_img" src="img/energy/product_thick_film09.png" alt="LED 崁燈"></a>
                            <p><a href="energy_led_chandelier_high_power.php">LED高功率崁燈</a></p>
                        </div>
                        <p class="ltt_box_product_description">
                            高演色性 CRI > 80
                        </p>
                    </div>

                    <div class="wow slideInRight ltt_box_product_33 ltt_box_product_right">
                        <div class="ltt_box_laser_05 ltt_box_product_img02">
                            <a href="energy_led_ar111.php"><img class="ltt_box_product_img" src="img/energy/product_thick_film11.png" alt="LED AR111"></a>
                            <p><a href="energy_led_ar111.php">LED AR111</a></p>
                        </div>
                        <p class="ltt_box_product_description">
                            100V～240V 50/60Hz
                        </p>
                    </div>


                </div>

            </section>
            <!--公司產品服務 ＥＮＤ-->


            <!-- footer -->
            <?php include 'footer.php'; ?>

        </div>

    </main>


    <!-- menu -->
    <?php include 'menu_ltt.php'; ?>


    <!-- search  -->
    <?php include 'search.php'; ?>


    <!-- right 各單位快速連結 桌機版+平板手機  -->
    <?php include 'right_service.php'; ?>

    <!-- 客服專線 -->
    <?php include 'service_wid.php'; ?>


    <!-- TOP-->
    <a href="" id="scroll-to-top"></a>


    <!-- 網站基本架構 -->
    <script src="js/jquery-2.1.1.js"></script>
    <script src="js/jquery.mobile.custom.min.js"></script>
    <script src="js/main.js"></script> <!-- Resource jQuery -->


    <!-- 滾動式底圖 -->
    <script async type="text/javascript" src="js/paraLasic.1.1.js"></script>
    <!--置頂-->
    <script type="text/javascript" src="js/illbeback.min.js"></script>
    <script type="text/javascript">
        $(function() {
            $("#scroll-to-top").illBeBack();
        });
    </script>


</body>

</html>